...
# Generated by: axon config init --interactive (static site)
...
EOF
        ...
        cat > "$OUTPUT_FILE" <<EOF
# AXON Deployment Configuration
# Generated by: axon config init --interactive (docker)
# Date: $(date +"%Y-%m-%d %H:%M:%S")
...
if [ "$INTERACTIVE" = false ]; then
    ...
    echo "  3. Validate: axon config validate --config ${CONFIG_BASENAME}"
else
    # Get first configured environment for example command
    FIRST_ENV="${ENV_NAMES[0]:-production}"

    echo "  1. Validate: axon config validate --config ${CONFIG_BASENAME}"
    if [ "$PRODUCT_TYPE" = "static" ]; then
